...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // ── BUSCAR API: limit=10000, PARALELO ────────────────────────────
  async function buscarAPI(dtIni, dtFim, funcao = '', silencioso = false) {
    if (!silencioso) log('Conectando à API...', 'linfo');

    // 1. Primeira página para saber o total
    ...
    chartBar = new Chart(el.getContext('2d'), {
      ...
      options: { responsive: true, maintainAspectRatio: false,
        ...
        scales: {
          ...
          y2: { position: 'right', grid: { display: false }, ticks: { color: '#f6a623', callback: v => (v >= 0 ? '+' : '') + v + 'h', font: { size: 9 } }, title: { display: true, text: 'Diferença', color: '#f6a623', font: { size: 9 } } }
        }
      }
    });
  }
  ...
  function renderEvo() {
    ...
    setEl('evoSubtitle', { dia: '01/01/2026 → ' + hoje + ' — diário', mes: '01/01/2026 → ' + hoje + ' — mensal', ano: 'Visão anual' }[evoModo]);
    ...
    chartEvo = new Chart(el.getContext('2d'), {
      data: { labels: labs, datasets: [
        ...
        { type: 'bar',  label: 'Diferença', data: difs, backgroundColor: difs.map(v => v >= 0 ? 'rgba(25,212,110,0.35)' : 'rgba(246,88,88,0.35)'), borderColor: difs.map(v => v >= 0 ? '#19d46e' : '#f65858'), borderWidth: 1, borderRadius: 2, yAxisID: 'y2' }
      ]},
      options: { responsive: true, maintainAspectRatio: false,
        plugins: { legend: { position: 'top', labels: { color: '#7a9cc8', boxWidth: 10, font: { size: 10 } } }, tooltip: { mode: 'index', intersect: false } },
        scales: {
  ...
  function renderDetalhamento(dados) {
    const linhaM = {};
    dados.forEach(p => {
      if (!linhaM[p.linha]) linhaM[p.linha] = { linha: p.linha, gar: mapaGar[p.linha]||'—', lote: mapaLote[p.linha]||'—', ttProg:0, ttReal:0, hrNProg:0, hrNReal:0, heProg:0, heReal:0, hnr:0, dobra:0, registros:[] };
  ...
  window.verLinha = function(linha) {
    ...
    row.registros.forEach(p => {
      ...
      if (!porDataTabela[chave]) porDataTabela[chave] = { data: p.data, tabela: p.tabela || '—', linha: p.linha, ttProg: 0, ttReal: 0, mot: 0, cob: 0 };
  ...
  $('btnConsultar')?.addEventListener('click', async () => {
    ...
    const tog = $('btnTogLog'); if (tog) tog.textContent = '👁 Ocultar Log';
  ...
  try {
    ...
    $('btnConsultar')?.click();
  } catch (e) { log('Erro na inicialização: ' + e.message, 'lerro'); }

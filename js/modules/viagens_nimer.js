...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  function renderMotivos(dados){
    ...
    if(el){if(chartMotivos)chartMotivos.destroy();const top6=motivos.slice(0,6);const outros=motivos.slice(6).reduce((s,m)=>s+m.cnt,0);const codLbls=top6.map(m=>m.cod+' - '+nomeM(m.cod));const vals=top6.map(m=>m.cnt);if(outros){codLbls.push('Outros');vals.push(outros);}const ttl=vals.reduce((a,b)=>a+b,0);chartMotivos=new Chart(el.getContext('2d'),{type:'doughnut',data:{labels:codLbls,datasets:[{data:vals,backgroundColor:CORES.slice(0,codLbls.length),borderWidth:0,hoverOffset:4}]},options:{responsive:true,maintainAspectRatio:false,cutout:'55%',plugins:{legend:{display:false},tooltip:{displayColors:false,callbacks:{title:function(){return '';},label:function(ctx){return ctx.label+': '+ctx.parsed+' ('+(ttl>0?(ctx.parsed/ttl*100).toFixed(1):0)+'%)';}}}}}}); }
    if(lista){const total=motivos.reduce((s,m)=>s+m.cnt,0);lista.innerHTML=motivos.map((m,i)=>{const cor=CORES[Math.min(i,CORES.length-1)];const pct=total>0?(m.cnt/total*100).toFixed(1):0;return`<div class="motivo-item"><span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:${cor};flex-shrink:0"></span><span style="color:var(--text);flex:1;margin:0 8px;font-size:12px">${m.cod} - ${nomeM(m.cod)}</span><span style="color:var(--warning);font-weight:700;min-width:36px;text-align:right">${fmt(m.cnt)}</span><span style="color:var(--muted);margin-left:8px;font-size:11px;min-width:40px;text-align:right">${pct}%</span></div>`;}).join('');}
  }
  ...
  function renderFiscaisTable(arr,col,asc){const sorted=sortArr(arr,col,asc);const tbody=$('tbFiscais');if(!tbody)return;tbody.innerHTML=sorted.length?sorted.map((f,idx)=>{const cobCor=f.cobertura>=90?'#19d46e':f.cobertura>=75?'#f6a623':'#f65858';const perf=f.cobertura>=90?'<span class="tag tag-ok">Bom</span>':f.cobertura>=75?'<span class="tag tag-warn">Regular</span>':'<span class="tag tag-err">Crítico</span>';return`<tr><td style="color:var(--muted);text-align:center">${idx+1}</td><td style="font-family:monospace;font-weight:700;color:#00d4ff">${f.re}</td><td>${badgeTurno(f.turno)}</td><td style="color:var(--text-secondary)">${f.primeiraHora}h</td><td style="color:#19d46e;font-weight:700">${fmt(f.marcadas)}</td><td style="color:var(--text-secondary)">${fmt(f.responsavel)}</td><td style="color:${f.naoMarcadas>0?'#f65858':'#19d46e'};font-weight:700">${fmt(f.naoMarcadas)}</td><td><div style="display:flex;align-items:center;gap:6px"><div style="height:7px;border-radius:3px;background:${cobCor};width:${Math.max(4,f.cobertura)}px"></div><span style="font-size:10px;font-weight:800;color:${cobCor}">${f.cobertura}%</span></div></td><td>${perf}</td></tr>`;}).join(''):'<tr><td colspan="9" style="text-align:center;color:var(--muted);padding:20px">Nenhum fiscal identificado</td></tr>';}

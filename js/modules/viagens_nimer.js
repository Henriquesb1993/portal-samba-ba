...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  function renderMotivos(dados){
    ...
    if(lista){const total=motivos.reduce((s,m)=>s+m.cnt,0);lista.innerHTML=motivos.map((m,i)=>{const cor=CORES[Math.min(i,CORES.length-1)];const pct=total>0?(m.cnt/total*100).toFixed(1):0;return`<div class="motivo-item"><span class="motivo-cod" style="background:var(--bg);color:${cor};border:1px solid var(--border)">${m.cod}</span><span style="color:var(--text);flex:1;margin:0 10px;font-size:12px">${nomeM(m.cod)}</span><span style="color:var(--warning);font-weight:700;min-width:36px;text-align:right">${fmt(m.cnt)}</span><span style="color:var(--muted);margin-left:8px;font-size:11px;min-width:40px;text-align:right">${pct}%</span></div>`;}).join('');}
  }
